...
public class BiomeDevonianGilboaLake extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:devonian_gilboa_lake")
	public static final BiomeGenCustom biome = null;
	public BiomeDevonianGilboaLake(ElementsLepidodendronMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomeDevonian {
		public BiomeGenCustom() {
			super(new BiomeProperties("Devonian Red Lake").setRainfall(0.7F).setBaseHeight(-0.575F).setHeightVariation(0.325F).setTemperature(1.5F).setWaterColor(9542514));
			setRegistryName("lepidodendron:devonian_gilboa_lake");
			topBlock = BlockClayRed.block.getDefaultState();
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 9; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					XENOCLADIA_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 1; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					STUMP_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
