...
public class BiomeDevonianCreekSwamp extends ElementsLepidodendronMod.ModElement {
	...
	public BiomeDevonianCreekSwamp(ElementsLepidodendronMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomeDevonian {
		public BiomeGenCustom() {
			super(new BiomeProperties("Devonian Swamp Creek").setRainfall(0.5F).setBaseHeight(-0.525F).setHeightVariation(0.0F).setTemperature(1.5F).setRainfall(0.5F));
			setRegistryName("lepidodendron:devonian_creek_swamp");
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 36; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					SILTY_DIRT_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 16; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					LUSH_DIRT_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS)) {
				...
				for (int j = 0; j < i; ++j)
				{
					...
					int l = rand.nextInt(16) + 8;
					BlockPos blockpos = worldIn.getHeight(pos.add(k, 0, l));
					ARCHAEOPTERIS_LOG_WATER_GENERATOR.generate(worldIn, rand, blockpos);
				}
	        }
