...
public class BiomeDevonianCreekMeadow extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:devonian_creek_meadow")
	public static final BiomeGenCustom biome = null;
	public BiomeDevonianCreekMeadow(ElementsLepidodendronMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomeDevonian {
		public BiomeGenCustom() {
			super(new BiomeProperties("Devonian Wet Meadow Creek").setRainfall(0.2F).setBaseHeight(-0.525F).setHeightVariation(0.0F).setTemperature(1.5F).setWaterColor(3906905));
			setRegistryName("lepidodendron:devonian_creek_meadow");
		...
		protected static final WorldGenGosslingia GOSSLINGIA_GENERATOR = new WorldGenGosslingia();
		protected static final WorldGenDiscalis DISCALIS_GENERATOR = new WorldGenDiscalis();
		protected static final WorldGenStachyophyton STACHYOPHYTON_GENERATOR = new WorldGenStachyophyton();
		protected static final WorldGenCrenaticaulis CRENATICAULIS_GENERATOR = new WorldGenCrenaticaulis();
		...
		protected static final WorldGenGravelPatch SILTY_DIRT_GENERATOR = new WorldGenGravelPatch(BlockCoarseSiltyDirt.block, 3);
...
//		protected static final WorldGenAsteroxylon ASTEROXYLON_GENERATOR = new WorldGenAsteroxylon();
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 36; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					DISCALIS_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 36; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					GOSSLINGIA_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 36; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					STACHYOPHYTON_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 36; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					CRENATICAULIS_GENERATOR.generate(worldIn, rand, pos.add(j, l, k));
				}
		...
		public EnumBiomeTypeDevonian getBiomeType() {
			return EnumBiomeTypeDevonian.Meadow;
		}
	}
