...
import net.pndevonian.world.biome.devonian.BiomeDevonianSpikes;

import javax.annotation.Nullable;
import java.util.Random;

@ElementsLepidodendronMod.ModElement.Tag
public class WorldDevonian extends ElementsLepidodendronMod.ModElement {
	public static int DIMID = LepidodendronConfig.dimDevonian;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("lepidodendron:devonian_portal")
	public static final BlockCustomPortal portal = new BlockCustomPortal();
	public static DimensionType dtype;
	public WorldDevonian(ElementsLepidodendronMod instance) {
		super(instance, 1590);
	}
	...
	public static class WorldProviderMod extends WorldProvider {
		@Override
		public void init() {
			this.biomeProvider = new net.pndevonian.world.dimension.devonian.BiomeProviderDevonian(this.world.getSeed(), this.world.getWorldInfo());
		...
		public Biome getBiomeForCoordsBody(final BlockPos pos, World worldIn)
		{
			...
			{
				return worldIn.provider.getBiomeProvider().getBiome(pos, BiomeDevonianOcean.biome);
			}
		}
		...
		public Biome getBiome(Chunk chunk, BlockPos pos, BiomeProvider provider)
		{
			...
			if (k == 255)
			{
				...
				Biome biome = world.isRemote ? BiomeDevonianOcean.biome : provider.getBiome(pos, BiomeDevonianOcean.biome);
				k = Biome.getIdForBiome(biome);
				chunk.getBiomeArray()[j << 4 | i] = (byte)(k & 255);
			}

			Biome biome1 = Biome.getBiome(k);
			return biome1 == null ? BiomeDevonianOcean.biome : biome1;
		}
		...
		public IChunkGenerator createChunkGenerator() {
			return new net.pndevonian.world.dimension.devonian.ChunkProviderDevonian(this.world, this.world.getSeed() - DIMID);
		}
		...
		public boolean canBlockFreeze(BlockPos pos, boolean byWater) {
			if (world.getBiome(pos) == BiomeDevonianSpikes.biome) {
				return false;
			}
	...
	public static class TeleporterDimensionMod extends Teleporter {
		...
		public boolean makePortal(Entity entityIn) {
			...
			//Are we above water?
			//System.err.println("Triggered portal at: " + j1 + " " + j1 + " " + );
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10+1, k11),
									...
									flag
											? BlockArchaeopterisPlanks.block.getDefaultState().getBlock().getDefaultState()
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11+1, k12),
								flag1 ? BlockArchaeopterisPlanks.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setTranslationKey("pf_devonian_portal");
			setRegistryName("devonian_portal");
			setLightLevel(0F);
		}
		...
		public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				Minecraft.getMinecraft().effectRenderer.addEffect(ParticlePNPortal.PortalParticleFactory.createParticle(world, d0, d1, d2, d3, d4, d5, 0.7695F, 0.4688F, 0.1758F));
			}
		}
			...
			if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
				...
				if (thePlayer.timeUntilPortal > 0) {
					...
					if (thePlayer.dimension == DIMID) {
						...
						ModTriggers.ENTER_DEVONIAN.trigger((EntityPlayerMP)thePlayer);
					}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockArchaeopterisPlanks.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockArchaeopterisPlanks.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockArchaeopterisPlanks.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
							.getBlock() != BlockArchaeopterisPlanks.block.getDefaultState().getBlock()) {
						this.height = 0;
						break;
					}
				}
