    ...
    public void useWand(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (levelEnchantment > 0) {
            ...
            if (world.isAirBlock(pos1)) {
                if (world.provider.getDimension() == LepidodendronConfig.dimDevonian) {
                    return;
                }
    ...
    public void goToNether(EntityTravelToDimensionEvent event) {
        if (LepidodendronConfig.oneWayPortalsNether && LepidodendronConfig.oneWayPortals) {
            if (event.getDimension() == -1) {
                ...
                if (event.getEntity().getEntityWorld().provider.getDimensionType().getId() == LepidodendronConfig.dimDevonian) {
                    if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
                        ...
                        if (thePlayer.dimension != event.getDimension()) {
                            ...
                            WorldDevonian.BlockCustomPortal.transferPlayerToDimensionPN(thePlayer.server.getPlayerList(), thePlayer, event.getDimension(), WorldDevonian.BlockCustomPortal.getTeleporterForDimension(thePlayer, entityIn.getPosition(), event.getDimension()));
                        }
                    }
                    event.setCanceled(true);
                }
